...
//
// 收件人是每次呼叫傳入的（check 的擁有者），不是設定值。
type Mailer struct {
	addr, from string
	auth       smtp.Auth
}

// FromEnv 從環境變數建 Mailer。沒設 SMTP_HOST 就回 nil。
func FromEnv() *Mailer {
	...
	m := &Mailer{addr: host + ":" + port, from: from}
...
func (m *Mailer) Send(to, subject, body string) error {
	...
	if err := smtp.SendMail(m.addr, m.auth, m.from, []string{to}, []byte(msg)); err != nil {
		return fmt.Errorf("寄信到 %s 失敗: %w", to, err)
	}
	return nil
}
